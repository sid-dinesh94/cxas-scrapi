        ...
        **kwargs: Any,
        ...
        if console_logging:
            ...
            for step_prog in eval_conv.steps_progress:
                ...
                print(
                    f"  {status_icon} {step_prog.step.goal[:80]} → {step_prog.status.value}"
                )
...
def cmd_run(args):
    ...
    if args.eval:
        ...
        for name, t in templates.items():
            ...
            if args.priority:
                ...
                if not tags:
                    print(
                        f"  WARNING: sim '{name}' has no tags — including anyway (add tags for proper filtering)"
                    )
    ...
    print(
        f"Running {len(test_cases)} evals x {runs} runs ({modality}, model: {model})"
    )
    if parallel > 1:
